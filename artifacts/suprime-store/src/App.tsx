...
type User = { id?: number; name: string; email: string; phone?: string; address?: string; city?: string; role: 'admin' | 'customer' };
...
function Home() {
  ...
  // 1. CARRILLO CON LOCALSTORAGE PERSISTENTE
  ...
  // PAGO CONTROLADO (CHECKOUT DE PAYPAL)
  ...
  return (
    <div className="store-shell min-h-[100dvh]">
            ...
            {currentUser ? (
              <div className="flex items-center gap-2 bg-[#e7e1d5] px-3 py-1.5 rounded-full">
                {currentUser.role === 'admin' && (
                  <button onClick={() => setAdminModalOpen(true)} title="Panel de Administración" className="p-1 text-[#ec684f]">
                    <Shield size={16} />
                  </button>
                )}
                <button onClick={() => setProfileModalOpen(true)} className="text-[11px] font-bold text-[#174f49] truncate max-w-[90px]">
              ...
              <button onClick={() => { setAuthEmail(''); setAuthPassword(''); setAuthModalOpen(true); }} className="icon-button relative rounded-full p-2" aria-label="Cuenta"><UserRound size={19} strokeWidth={1.7} /></button>
            )}
      ...
      {mobileMenu && (
        <div className="fixed inset-0 z-[60] flex min-h-[100dvh] flex-col bg-[#174f49] px-6 py-7 text-[#f6f0e6] md:hidden">
          <div className="flex items-center justify-between border-b border-[#286b69] pb-4">
            ...
            <button onClick={() => setMobileMenu(false)} className="rounded-full p-2 hover:bg-[#1e4a49]"><X size={24} /></button>
          </div>
          <div className="mt-8 flex flex-col gap-6 font-display text-3xl">
            {(['Todo', 'Mujer', 'Hombre', 'Niños', 'Casa', 'Bienestar'] as Category[]).map((category) => (
              <button className="text-left hover:text-[#ec684f] transition-colors" key={category} onClick={() => { setSelectedCategory(category); setMobileMenu(false); document.getElementById('productos')?.scrollIntoView({ behavior: 'smooth' }); }}>{category}</button>
            ))}
          </div>
        </div>
      )}
          ...
          <div className="flex h-full w-full max-w-md flex-col bg-[#f6f0e6] p-6 shadow-xl">
            ...
            {cart.length > 0 && (
              <div className="border-t border-[#e7e1d5] pt-4">
                ...
                <button onClick={handleCheckoutClick} className="w-full rounded-full bg-[#174f49] py-4 text-center text-[12px] font-bold uppercase tracking-widest text-[#fff4e7] transition-colors hover:bg-[#ec684f]">
      ...
      {authModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#174f49]/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-[28px] bg-[#f6f0e6] p-6 shadow-2xl md:p-8">
            ...
            <p className="mt-1 text-[13px] text-[#596660]">{isRegisterMode ? 'Únete a Suprime para realizar tus compras' : 'Ingresa con tus credenciales'}</p>

            <form onSubmit={handleAuthSubmit} className="mt-6 flex flex-col gap-4">
              {isRegisterMode && (
                <input type="text" placeholder="Tu nombre completo" value={authName} onChange={(e) => setAuthName(e.target.value)} required className="rounded-full bg-[#ebe4d8] px-5 py-3 text-[13px] text-[#174f49] outline-none" />
              )}
              <input type="email" placeholder="Tu correo electrónico" value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} required className="rounded-full bg-[#ebe4d8] px-5 py-3 text-[13px] text-[#174f49] outline-none" />
              <input type="password" placeholder="Contraseña" value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} required className="rounded-full bg-[#ebe4d8] px-5 py-3 text-[13px] text-[#174f49] outline-none" />
      ...
      {profileModalOpen && currentUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#174f49]/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-md rounded-[28px] bg-[#f6f0e6] p-6 shadow-2xl md:p-8">
            <button onClick={() => setProfileModalOpen(false)} className="absolute right-4 top-4 rounded-full bg-[#e7e1d5] p-2 text-[#174f49] hover:bg-[#ded6c7]"><X size={18} /></button>
            ...
            <div className="mt-6 flex gap-2">
              <button onClick={logout} className="w-full rounded-full bg-[#ec684f] py-3 text-[12px] font-bold uppercase tracking-wider text-white">Cerrar Sesión</button>
            </div>
          </div>
        </div>
      )}
      ...
      {adminModalOpen && currentUser?.role === 'admin' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#174f49]/60 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-[28px] bg-[#f6f0e6] p-6 shadow-2xl md:p-8">
            <button onClick={() => setAdminModalOpen(false)} className="absolute right-4 top-4 rounded-full bg-[#e7e1d5] p-2 text-[#174f49] hover:bg-[#ded6c7]"><X size={18} /></button>
            <div className="flex items-center gap-2 text-[#ec684f] font-bold text-[12px] uppercase tracking-wider"><Shield size={18} /> Panel de Administración</div>
                ...
                {productsList.map(p => (
                  <div key={p.id} className="flex items-center justify-between bg-[#ebe4d8] p-3 rounded-xl text-[12px]">
                    <div>
                      ...
                      <span className="block text-[10px] text-[#596660]">{p.category} — {money(p.price)}</span>
                    </div>
                    <span className="bg-[#174f49] text-[#fff4e7] px-2 py-0.5 rounded-full text-[10px]">Stock: {p.stock || 0}</span>
                  </div>
                ))}
              </div>
            </div>
